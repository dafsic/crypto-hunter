...
import (
	...
	pg "github.com/dafsic/crypto-hunter/postgres"
...
func (dao *DaoImpl) CreateOrder(ctx context.Context, order *model.Order) error {
	query, args := pg.ToSQL(pg.Insert(orderTable).
		Columns("order_id", "bot", "exchange", "pair", "price", "amoount", "side", "multipler", "order_status").
...
func (dao *DaoImpl) GetOrder(ctx context.Context, id int) (*model.Order, error) {
	...
	query, args := pg.ToSQL(pg.
		Select("id", "order_id", "bot", "exchange", "pair", "price", "amoount", "side", "multipler", "order_status", "created_at", "updated_at").
		From(orderTable).
		Where(sq.Eq{"id": id}))
